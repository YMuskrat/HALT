...
A contribution needs a named strategy, source/version, configuration, requirements,
...
1. Run `halt new-method my_stopper --output runs/my_stopper` and install the generated
   package with `python -m pip install -e runs/my_stopper`.
2. Edit its method class and method card. Declare capabilities, exact signal semantics,
   boundary policy and finalization recipe. Use the `halt.methods` entry point group
   and plugin API version `1`. IDs must be unique lowercase snake_case.
...
Methods implement `reset(context)`, `observe(event)` and `decide()`. They must not
